...
func (p *ModelProviderResolverPlugin) propose(ctx context.Context, request *requesthandling.InferenceRequest, modelInfo *externalModelInfo) error {
	...
	if len(endpoints) == 0 {
		log.FromContext(ctx).V(logutil.DEBUG).Info("hub mode PROPOSE: no eligible endpoints, skipping subset hint",
...
// matches the destination. Disabled refs (weight <= 0) are skipped so a stale or
// spoofed destination header can never route to a provider PROPOSE never
// advertised. Ports are stripped from both sides before comparison.
